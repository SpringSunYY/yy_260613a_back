...
or with uv
uv run --with simple-ddl-parser convertor.py postgres > ../postgresql/litchi.sql                                         239ms  四  5/22 21:03:16 2025
uv run --with simple-ddl-parser convertor.py sqlserver > ../sqlserver/litchi.sql
uv run --with simple-ddl-parser convertor.py kingbase > ../kingbase/litchi.sql
uv run --with simple-ddl-parser convertor.py opengauss > ../opengauss/litchi.sql
uv run --with simple-ddl-parser convertor.py oracle > ../oracle/litchi.sql
uv run --with simple-ddl-parser convertor.py dm8 > ../dm/litchi-dm8.sql
"""
...
def main():
    parser = argparse.ArgumentParser(description="荔枝系统数据库转换工具")
    parser.add_argument(
        "type",
        type=str,
        help="目标数据库类型",
    ...
    sql_file = pathlib.Path("../mysql/litchi.sql").resolve().as_posix()
    convertor = None
